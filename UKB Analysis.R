...
PL_estimates = PL_SE = pairwise_estimates = pairwise_SE_boot2 = pairwise_SE_boot3 = matrix(NA,nrow = 24,ncol = 8)
...
for(i in 1:24)
{
  ...
  PL_estimates[i,] = coef(fit1)
  PL_SE[i,] = sqrt(diag(vcov(fit1)))
...
# Deriving adjusted and raw p-values

PvaluesPL = pnorm(PL_estimates[,8] / PL_SE[,8],lower.tail = F)
PvaluesPairwiseBoot2 = pnorm(PairwiseEst[,8] / pairwise_SE_boot2[,8],lower.tail = F)
PvaluesPairwiseBoot3 = pnorm(PairwiseEst[,8] / pairwise_SE_boot3[,8],lower.tail = F)

PvaluesPLBH = p.adjust(PvaluesPL,method = "BH")
PvaluesPairwiseBoot2BH = p.adjust(PvaluesPairwiseBoot2,method = "BH")
PvaluesPairwiseBoot3BH = p.adjust(PvaluesPairwiseBoot3,method = "BH")
